...
def load_model(model_type, num_classes=23):
    try:
        ...
        if model_type == "Base Model":
            ...
            model = base_model
            model.load_state_dict(torch.load('models/base_model_state.pth', map_location=torch.device('cpu')))
        else:
            # Настройка улучшенной модели
            model = ImprovedModel(base_model, num_classes)
            model.load_state_dict(torch.load('models/improved_model_state.pth', map_location=torch.device('cpu')))
